...
/**
 * 将 Blob 转成 base64 的 data URL
 * @param  blob 任意二进制对象（图片、音频、PDF 等均可）
 * @returns  形如 data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...
 */
export function blobToDataURL(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result as string);   // reader.result 就是 dataURL
    reader.onerror = () => reject(reader.error);
    reader.readAsDataURL(blob);                     // 一次性读成 data:... 的形式
  });
}