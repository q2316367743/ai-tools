...
import {openAddAiChatGroupDialog} from "@/pages/chat/modal/AddAiChatGroup";
import {isDark, useAiChatGroupStore, useAiChatListStore} from "@/store";
import {activeKey, autoHideCollapsed, collapsed, toggleCollapsed} from './model';
import {AiChatGroup} from "@/types";
import {onRenameGroup, onRemoveGroup} from "@/pages/chat/components/HomeContext";
import {openHomeChatSearch} from "@/pages/chat/components/HomeChatSearch";
import {LocalNameEnum} from "@/global/LocalNameEnum";
import {KeyValueUtil} from "@/utils/utools/KeyValueUtil";
import HomeWelcome from "@/pages/chat/pages/welcome/HomeWelcome.vue";
import HomeGroup from "@/pages/chat/pages/group/HomeGroup.vue";
import HomeChat from "@/pages/chat/pages/chat/HomeChat.vue";
import HomeTemp from "@/pages/chat/pages/temp/HomeTemp.vue";
import HomeChatList from "@/pages/chat/components/HomeChatList.vue";
...
tryOnMounted(async () => {
  ...
  if (route.query.preload) {
    const id = await useAiChatListStore().post(
      ...
      `${route.query.preload}`,
      // 默认使用豆包
      KeyValueUtil.getItemByDefault(LocalNameEnum.KEY_AI_MODEL, "doubao-1.5-pro-32k"));
