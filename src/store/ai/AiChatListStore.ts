...
import {AiChatContent, AiChatItem, AiChatList, AiChatWrap} from "@/types";
import {
  DbList,
  getFromOneByAsync,
  listByAsync,
...
import {renderChat} from "@/pages/chat/model";
...
export const useAiChatListStore = defineStore('ai-chat-list', () => {
  ...
  const post = async (groupId: string, question: string, model: string): Promise<string> => {
